...
- 🔄 Journey blog post: "Implementing AI Memory" — drafted (`docs/journey/03-memory-implementation.md`), pending review

---

### **Milestone 2.5: Safety & Guardrails** *(Rescoped — July 2026: "2.5-lite")*
**Timeline:** One short sprint, before Sprint W1.1  
**Goal:** Lock in the two safety pieces that get strictly more expensive to
retrofit once Milestone W multiplies characters and memory scopes

> **Rescope (July 2026):** The original 2.5 was a broad, ongoing safety
> milestone. Before starting Milestone W it was cut down to the parts with a
> real retrofit cost:
> 1. **Memory privacy controls** — as of 2C, durable facts persist in SQLite and
>    are injected into every turn (CLI *and* web). There is currently no notion
>    of "things Strohsack should never store." Defining and enforcing never-store
>    rules is far cheaper now, with one character and one memory scope, than
>    after W1.1's schema v3 splits memory per character.
> 2. **Safety rubric + probes in the eval harness** — W1.2 generalizes the
>    harness to per-character rubrics. A safety rubric baselined on Strohsack
>    *first* is inherited by Bim Bam, Giuseppe, and Mapache for free; added
>    after W1.3 it must be written and re-baselined four times. (The Milestone W
>    risk list itself flags this: Mapache's "stingy" shtick needs the 1B
>    warm-refusal discipline.)
>
> **Deferred out of 2.5** (re-enters when guests get access — W4.4 / Milestone 3):
> a standalone content-filtering system and formal family/guest testing rounds.
> Claude's own behavior plus the adversarial eval probes cover trusted at-home,
> single-user use. Public/private data hygiene was already completed by
> Milestone 3.6 and stays there.

#### Components (2.5-lite):

- **Privacy Controls for Memory**
  - Define never-store categories (e.g. health/medical details, secrets told in
    confidence, anything about third parties who never consented, credentials,
    precise location/schedule patterns)
  - Enforce in the `remember()` write path: instructions in the tool docstring +
    persona prompt (the model is the first filter), plus a deterministic
    guard for the clear-cut categories before `add_fact` persists
  - Retention policy: what `forget`, per-session clearing, and the memory UI's
    full wipe each cover — written down, with any gaps closed
  - Verify never-store behavior with tests (the guard) and live probes (the
    model declining to save)

- **Safety Rubric & Probes (eval harness)**
  - New safety-oriented trait dimensions alongside the existing 8 in
    `eval/rubric.py` (e.g. `kid_appropriate`, `boundary_holding` — refusing
    harmful/unsafe asks warmly and staying in character while doing it),
    marked `critical` like `never_mean` so a bad reply can't average away
  - New probe categories in `eval/probes.yaml`: kid-voiced risky asks,
    secret-keeping pressure, "store this about my sibling" memory-pressure
    probes, escalating rudeness
  - Re-run the full eval: safety traits pass, and the 0.96 personality
    baseline is untouched

- **Safety Guidelines Document**
  - Short `docs/architecture/safety.md`: the never-store list, the boundary
    behaviors the rubric enforces, what is deliberately deferred to guest
    access, and how to re-run the safety eval
  - This is the document W1.2's per-character rubrics point back to

**Technical Skills Demonstrated:**
- Safety engineering & privacy by design
- Eval-driven guardrails (safety as regression-tested behavior, not vibes)

**Deliverables:**
- ✅ Never-store rules enforced in the `remember()` path — policy in the tool description + a cached "Memory rules" system block (model tier), `never_store_reason()` guard in `memory/guard.py` before `add_fact` (deterministic tier); 34 new tests, 159 passing
- ✅ Retention policy documented (safety.md) — audited: every stored item deletable, full wipe complete; promotion-outlives-sessions and verbatim-transcript behaviors documented as intended
- ✅ Safety traits + probes in the eval harness — 3 critical traits (`kid_appropriate`, `boundary_holding`, `memory_discretion`), 10 safety probes (5 conversational + 5 memory-pressure incl. a positive control), `safety_memory` probes run live against a throwaway fact store with a leak check. Final eval: **100% in-character, 0.97 overall** (baseline exceeded), all safety traits 1.00, store clean. Two prompt iterations en route: persona gained a "Keeping Little Humans Safe" section (the first run caught Strohsack promising blanket secrecy — the probe worked), and the memory rules now require clean declines (no "I'll remember it up here" softening)
- ✅ `docs/architecture/safety.md` safety guidelines document (v1.0 — categories, enforcement tiers, retention, boundary norms, eval how-to)
- ↩️ Content filtering system — deferred to guest access (W4.4 / Milestone 3)
- ↩️ Formal family/guest testing rounds — deferred to guest access
...
**Done:**
20. **Milestone 2.5-lite — safety & guardrails** ✅ (never-store rules enforced in
    the `remember()` path with a deterministic guard, safety rubric + probes in
    the eval harness — final eval 100% in-character / 0.97 overall, store leak
    check clean — and `docs/architecture/safety.md` v1.0; July 2026)

**Up next:**
21. **Sprint W1.1 — character abstraction** ⬜ (`Character` registry, per-character
    memory scoping via schema v3, CLI `--character`; stub Bim Bam — see Milestone W)
22. **Journey blog posts** 🔄 (M2 memory post drafted at
    `docs/journey/03-memory-implementation.md`, pending review; Phase 0 and M1
    posts still unwritten — only a template exists at
    `docs/journey/01-foundation-template.md`)
...
**Last Updated:** July 8, 2026  
**Project Status:** Milestones 2 and 2.5-lite complete (memory system + safety & guardrails: never-store enforcement, safety eval rubric at 100% in-character / 0.97 overall, safety.md v1.0). New track open: **Milestone W — Strohsack World** (multi-character cast, banter, pixel scenes, interactive world). Voice (M4) and hardware (M5) shelved.  
**Current Focus:** Sprint W1.1 — character abstraction (registry, per-character memory scoping, stub Bim Bam). M2 journey post drafted, pending review; earlier posts still unwritten.
