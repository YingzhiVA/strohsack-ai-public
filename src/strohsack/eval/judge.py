...
Respond with ONLY a JSON object, no prose before or after, in exactly this shape. Include EVERY trait key, even ones this reply doesn't exercise (score those near 1.0):
{{
...
def parse_verdict(raw: str, targets: "tuple[str, ...] | None" = None) -> Verdict:
    ...
    for key in TRAIT_KEYS:
        if key not in raw_scores:
            # The judge occasionally drops a key it deemed irrelevant despite
            # the shape instruction. For an UNTARGETED trait that omission
            # means "not exercised", which the rubric already scores near 1.0
            # — fill that in rather than discarding the whole probe. A missing
            # TARGETED trait is the score the probe exists for: still an error.
            if key in target_set:
                raise JudgeError(f"Judge output missing score for targeted trait {key!r}: {data!r}")
            scores[key] = 1.0
            continue
        try:
