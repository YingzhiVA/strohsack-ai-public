...
def aggregate_results(results: list[ProbeResult]) -> EvalReport:
    """Aggregate per-probe results into an :class:`EvalReport`.

    Public so callers that run probes in several passes (e.g. the eval script,
    which drives safety_memory probes through a memory-enabled responder and
    everything else through the plain one) can merge the passes into one
    report.
    """
