...
class StrohsackClient:
    ...
    def _build_remember_tool(self):
        ...
        def remember(note: str) -> str:
            ...
            their name, where they live, what they do, preferences and dislikes,
            people or pets in their life, ongoing projects, or an inside joke the
            two of you build. Do NOT save passing small talk, one-off questions,
            anything they'd be uncomfortable seeing written down, or facts you
            already see in your notes. Write each note as one short, standalone
            sentence.
    ...
    def _system_blocks(self) -> list[dict]:
        ...
        header) then serves it at ~0.1x input cost on a cache hit. Durable facts,
        which change whenever ``remember()`` fires, go in a *separate* block
        after the breakpoint so saving a fact doesn't invalidate the cached
        persona prefix. Tools render before ``system``, so the ``remember()``
        schema is cached in the same prefix. (Measured at ~2.3-3.0K tokens, above
        Sonnet 4.6's 2048-token cache minimum, so the cache actually engages.)
        """
        blocks: list[dict] = [
            {
                "type": "text",
