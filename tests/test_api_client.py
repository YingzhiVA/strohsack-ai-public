...
class TestRespondWithMemory:
    ...
    def test_injects_facts_after_cached_persona_block(self) -> None:
        ...
        # Persona block first (cache breakpoint), facts in a separate block AFTER
        # it (no cache_control) so saving a fact won't invalidate the persona cache.
        assert system[0]["text"] == "You are a bear."
        assert system[0]["cache_control"] == {"type": "ephemeral"}
        assert "- Name is Yingzhi" in system[1]["text"]
        assert "- Loves oak honey" in system[1]["text"]
    ...
    def test_no_facts_block_when_store_empty(self) -> None:
        ...
        client.respond([{"role": "user", "content": "hi"}])
        system = mock.beta.messages.tool_runner.call_args.kwargs["system"]
        # Just the cached persona block — no facts block appended.
        assert len(system) == 1
        assert system[0]["text"] == "You are a bear."
        assert system[0]["cache_control"] == {"type": "ephemeral"}
